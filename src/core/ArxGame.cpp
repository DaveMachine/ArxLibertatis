...
renderend:
			...
			if (!PLAY_LOADED_CINEMATIC && !CINEMASCOPE && !BLOCK_PLAYER_CONTROLS && ARXmenu.currentmode == AMCM_OFF)
			{
...
void ArxGame::render_player()
{
	if (inter.iobj[0] && inter.iobj[0]->animlayer[0].cur_anim)
	{
		...
		if (inter.iobj[0]->invisibility > 0.9f) 
		{
			inter.iobj[0]->invisibility = 0.9f;
		}
